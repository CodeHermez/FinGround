...
Create an empty PostgreSQL database named **`heliumdb`** using whichever client you have available, e.g. with `psql`:
...
On startup the app automatically runs any pending migrations and seeds demo data (see below) if the database is empty — you don't need a separate seed command.

The API listens on **http://localhost:5000** (configured via `Urls` in `appsettings.json`).
...
A pre-seeded admin account and two funded accounts are created automatically the first time you run the app against an empty database:
...
| Auth | `POST /api/auth/login` | 5 attempts / 60 sec per IP (sliding window) |
...
A `Dockerfile` is provided at [`API/Dockerfile`](API/Dockerfile) for containerized builds; it still requires a reachable PostgreSQL instance and the same environment configuration described above.
